#ifndef MTHREAD_SCHEDULER_FIFO_H
...
struct fifo_scheduler_s {
  ...
  void (*find_next_thread)(
      struct mthread_vp_s<
          struct fifo_scheduler_s<SchedLock, Lock, LoadBalancer>> *) =
      [](struct mthread_vp_s<
          struct fifo_scheduler_s<SchedLock, Lock, LoadBalancer>> *vp) {
  ...
  // Have to insert thread at the beginning of the list because current is the
  ...
  void (*insert_new_thread)(
      ...
      [](struct mthread_vp_s<
        ...
        if (thread->attr.vp == vp->id) {
          ...
          struct mthread_vp_s<struct fifo_scheduler_s<
              SchedLock, Lock, LoadBalancer>> *remote_vp =
              vp_list<struct fifo_scheduler_s<SchedLock, Lock,LoadBalancer>, Lock>.vp_vector
                  [thread->attr.vp];
  ...
  int (*loadbalancer)(
      ...
      [](struct mthread_vp_s<
             struct fifo_scheduler_s<SchedLock, Lock, LoadBalancer>> *vp,
         mthread_thread_t *thread) {
        return vp->scheduler.balancer.get_vp_id();
      };
...
// END Define new FIFO scheduler

#define MTHREAD_SCHEDULER_FIFO_H
#endif
