...
- Le **% CPU ≈ 99-100%** (jamais >100%) → confirme qu'un seul cœur travaille.
- Le temps **augmente linéairement** avec le nombre de threads : 2 threads = 2× le temps de 1 thread, 4 threads = 4× etc. Il n'y a **aucun gain**, c'est même **plus lent** car on fait N fois plus de travail séquentiellement.
...
- Le **% CPU >> 100%** : 187% pour 2 threads, 372% pour 4 threads → les threads s'exécutent sur **plusieurs cœurs simultanément**.
- Le **temps réel reste quasi constant** entre 1, 2 et 4 threads (~0.45-0.53s) alors que la charge de travail totale est multipliée par N → **parallélisme réel effectif**.
...
| % CPU avec 4 threads | ~99% (1 cœur) | ~372% (4 cœurs) |
| Temps réel (4 threads) | 2.33s (×4 vs 1 thread) | 0.53s (≈ même que 1 thread) |
...
- Le temps total est **toujours ≈ 2 s**, quel que soit le nombre de threads → les `pth_sleep()` s'exécutent **en parallèle**.
- Tous les threads démarrent leur sleep en même temps et se réveillent en même temps.
- Avec 8 threads, le pire cas serait 16 s (séquentiel), mais on obtient 2 s → **facteur ×8** de gain.
...
- Le temps total est **toujours ≈ 2 s**, quel que soit le nombre de threads → les `sleep()` s'exécutent **en parallèle**.
...
## Partie IV — Découverte du code de la bibliothèque mthread
...
> 📁 **Fichier source :** `mthread/tests/test_mutex_q19_q22.cpp`
...
cd mthread/build
cmake ..
make test_mutex_q19_q22
./tests/test_mutex_q19_q22
```
