...
int main(int argc, char** argv)
{
  ...
  /* ---- Version parallèle par tâches (Q.3) ---- */
  int nb_tasks = omp_get_max_threads() * 4;  /* surcharge de tâches pour équilibrage */
  fprintf(stdout, "\n=== Task-based SpMV (nb_tasks=%d) ===\n", nb_tasks);

  /* Réinitialiser x */
  for(i = 0; i < nrows; ++i)
    x[i] = 1. * i ;

  double duration_task = 0.;
  for(i = 0; i < nb_test; ++i)
  {
    for(j = 0; j < nrows; ++j) y[j] = 0.;
    t0 = get_elapsedtime();
    mult_CSR_task(A, x, y, nb_tasks);
    t1 = get_elapsedtime();
    duration_task += (t1 - t0);
  ...
  fprintf(stdout, " Time : %f\n", duration_task);
  fprintf(stdout, " MFlops : %.2f\n", flop_csr / (duration_task/nb_test)*1E-6);
  fprintf(stdout, "AvgTime : %f\n", duration_task/nb_test);
  fprintf(stdout, "\nSpeedup : %.2fx\n", duration / duration_task);
