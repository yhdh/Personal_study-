...
int main(){
   ...
   */
    ...
    int nb_threads = omp_get_max_threads(); // Pour avoir le nombre de threads
    
    // Initialisation du tableau temporaire pour accumuler les forces
    double **tmp_forces; 
    tmp_forces = (double **) malloc(nb_threads * sizeof(double *));
    for (int i = 0; i < nb_threads; ++i) 
    {
      tmp_forces[i] = (double *) malloc(npart*3 * sizeof(double));
      dfill(3*npart, 0.0, tmp_forces[i], 1);
    }
   ...
   */
     ...
     */
      forces(npart, x, f, side, rcoff, tmp_forces, nb_threads); 
